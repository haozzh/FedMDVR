...
class Client(DistributedTrainingDevice):
    ...
    def __init__(self, model, args, trn_x, trn_y, dataset_name, id_num=0):
        ...
        self.W_pre = {name: torch.zeros(value.shape).to(self.args.device) for name, value in self.W.items()}
    ...
    def train_cnn(self, server, m, iter):
        ...
        for j in range(self.args.local_ep):
            ...
            for i in range(self.local_epoch):
                ...
                self.W_pre = copy.deepcopy(self.W)
                self.gradient = {name: value.grad for name, value in self.model.named_parameters()}
                add_mome2(target=self.W, source1=self.W, source2=self.gradient, beta_1=1, beta_2=  self.args.coe_fedgamma)
                ...
                if  self.args.method == 'fedgamma':
                    self.gradient = {name: value.grad for name, value in self.model.named_parameters()}
                    add_mome2(target=self.W, source1=self.W_pre, source2=self.gradient, beta_1=1, beta_2 = - self.args.lr)
                    add_mome2(target=self.W, source1=self.W, source2=self.unbias, beta_1=1, beta_2= - self.args.lr)
