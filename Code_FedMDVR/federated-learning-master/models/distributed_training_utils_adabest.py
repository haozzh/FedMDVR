...
class Client(DistributedTrainingDevice):
    ...
    def train_cnn(self, server, m, iter):
        ...
        if self.args.method == 'fedavg' or self.args.method == 'adabest':
    ...
    def compute_weight_update(self, server, m, iter):
        ...
        if self.args.method == 'adabest':
            add_mome2(target=self.hi, source1=self.hi, source2=self.dW, beta_1=1 / (iter - self.t_prime), beta_2= - self.args.adacoe)
            self.t_prime = iter
...
class Server(DistributedTrainingDevice):
    ...
    def computer_weight_update_down_dw(self, clients, iter):
        ...
        if self.args.method == 'adabest':
            ...
            scale_ts(target=self.h, source=self.dW, scaling= - self.args.ada_beta)
            add_mome2(target=self.W, source1=self.W, source2= self.h, beta_1=1, beta_2=-1)
