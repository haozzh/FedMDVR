...
class Client(DistributedTrainingDevice):
    ...
    def compute_bias(self, server, w_glob_keys):
    ...
    def train_cnn(self, server, m, iter, w_glob_keys):

        self.model.train()


        if self.args.method == 'fedavg' or self.args.method == 'fedpvr':
                ...
                if self.args.method == 'fedpvr':
                    ...
                    for name in self.gradient:
                        if name in w_glob_keys:
                            self.W[name].data = self.W[name].data.clone() - self.args.lr * self.gradient[name].data.clone()
                        else:
                            self.W[name].data = self.W[name].data.clone() - self.args.lr * (self.gradient[name].data.clone() + self.unbias[name].data.clone())
    ...
    def compute_weight_update(self, server, m, iter, w_glob_keys):
        ...
        # W = SGD(W, D)
        self.train_loss = self.train_cnn(server, m, iter, w_glob_keys)
